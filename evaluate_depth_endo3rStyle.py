...
def get_args_parser():
    ...
    parser.add_argument('--endo3r_scale', action='store_true', help='Use Endo3R-style sequence-level scaling (scale once per sequence instead of per frame)')
    return parser
    ...
    if args.data_type == 'scared':
        for dir in datasets_dir:
            ...
            for key in all_scenes:
                ...
                # output_path = "./output/" + model_path.split('/')[-1].split('.')[0] + '/' + \
            ...
            args_list = ['python', 'eval/depth_evaluation.py','--data_root', args.data_root,
                        '--output_path', './outputs/' + model_path.split('/')[-1]]
            if args.endo3r_scale:
                args_list.append('--endo3r_scale')
            print(args_list)
